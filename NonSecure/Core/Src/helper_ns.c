...
void gpio_init_button(){
	...
	// I first set the pull-down before setting pin as input to prevent
	// spurious readouts which might be possible between setting the pin as input
	// and activating the pull-down register.

	// (1) Activate pull-down resistor (PUPD7 == 0b10)
	SET_BIT(gpio->PUPDR, GPIO_PUPDR_PUPD13_1);

	// (2) Set pin mode to input (MODE7 == 0b00)
...
bool_t s_msg_build_header(byte_t msg_type, s_msg_t* o_s_msg){
	...
	if(msg_type == SERVER_PAYLOAD_SIGNUP_REQ){
			...
			uint8_t dev_cap_len = strlen(dev_state_ns.dev_cap);
...
bool_t s_msg_build(byte_t msg_type, req_t* req_state, s_msg_t* o_s_msg){
	...
	case SERVER_PAYLOAD_SIGNUP_REQ:
		memcpy(o_payload, &dev_state_ns.dev_type, LEN_SERVER_PAYLOAD_SIGNUP_REQ);		// NOTE: Here, LEN_SERVER_PAYLOAD_SIGNUP_REQ
																						// is actually a lower bound for the payload length,
																						// which is the length of the device type (2 bytes)
		memcpy(o_payload + 2, dev_state_ns.dev_cap, strlen(dev_state_ns.dev_cap));
		return TRUE;

	case SERVER_PAYLOAD_AUTH_REQ:
		memcpy(o_payload, &dev_state_ns.dev_id, DEVICE_ID_LEN);
...
bool_t gw_msg_build(byte_t msg_type, req_t* req_state, gw_msg_t* o_gw_msg){
	...
	case GATEWAY_PAYLOAD_RESPONSE:
		memcpy(o_payload, req_state->response, LEN_GATEWAY_PAYLOAD_RESPONSE);
		return TRUE;
...
//	o_s_msg->msg_len = HEADER_LEN + LEN_SERVER_PAYLOAD_AUTH_REQ;
	s_msg_build_header(SERVER_PAYLOAD_AUTH_REQ, o_s_msg);

	// o_s_msg->msg_buf has format: | header (3 bytes) | device_id (4 bytes) |challenge (42 bytes) |

	byte_t* challenge_ptr = o_s_msg->msg_buf + HEADER_LEN;
	memcpy(challenge_ptr, &dev_state_ns.dev_id, DEVICE_ID_LEN);
	memcpy(challenge_ptr + DEVICE_ID_LEN, req_state->challenge, CHALLENGE_LEN);
}

//void init_req_state(req_t* req_state){
//
//	// (1) Set request to state of challenge but not authenticated
//
//	// (2)
//	req_state->
//
